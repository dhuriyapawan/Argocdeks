...
module "vpc" {
  ...
  single_nat_gateway    = false # Multi-NAT in Production for AZ redundancy
  tags                  = local.tags
}
...
module "rds" {
  ...
  kms_key_arn             = module.kms.key_arn
  multi_az                = true # Enabled Multi-AZ for failover support
  skip_final_snapshot     = false
  backup_retention_period = 30
  deletion_protection     = true # Protection from accidental deletion
  tags                    = local.tags
}
...
module "redis" {
  ...
  num_cache_clusters  = 2 # Multi-node Redis replication for load balancing
  database_subnet_ids = module.vpc.database_subnets
...
module "route53" {
  ...
  subdomain    = "" # Root Domain mapping for production
